...
void SPI_CS_Low()
  {
	...
	//Chip_GPIO_WritePortBit(LPC_GPIO,0,27,false);
...
SD_BOOL SD_Init (void)
{
    ...
    SD_DeSelect();
    Chip_SPI_CalClkRateDivider(LPC_SPI0,400000);                          // SPI_ConfigClockRate (SPI_CLOCKRATE_LOW);
    for (i = 0; i < 10; i++)    Chip_SPI_SendMidFrame(LPC_SPI0,0xFF);
...
init_end:              
   SD_DeSelect();
    ...
    {
        Chip_SPI_CalClkRateDivider(LPC_SPI0,25000000); /////////////////////SPI_ConfigClockRate (SPI_CLOCKRATE_HIGH);
        return (SD_TRUE);
    }
}
...
uint8_t SD_SendCommand (uint8_t cmd, uint32_t arg, uint8_t *buf, uint32_t len) 
{
   ...
   Chip_SPI_SendMidFrame(LPC_SPI0,cmd | 0x40);// SPI_SendByte (cmd | 0x40);
     Chip_SPI_SendMidFrame(LPC_SPI0,arg >> 24);                                         //SPI_SendByte (arg >> 24);
     Chip_SPI_SendMidFrame(LPC_SPI0,arg >> 16);                                                          //            SPI_SendByte (arg >> 16);
      Chip_SPI_SendMidFrame(LPC_SPI0,arg >> 8);                                                                                //     SPI_SendByte (arg >> 8);
        Chip_SPI_SendMidFrame(LPC_SPI0,arg);                                                                                      //SPI_SendByte (arg);
       Chip_SPI_SendMidFrame(LPC_SPI0,crc_stop);                                                                           //SPI_SendByte (crc_stop); /* Valid or dummy CRC plus stop bit */
    ...
    { 
        if (SD_SendCommand (WRITE_MULTIPLE_BLOCK, sect, NULL, 0) == R1_NO_ERROR)
        {
            ...
            /* Send Stop Transmission Token. */
             Chip_SPI_SendMidFrame(LPC_SPI0,0xFD);                              //SPI_SendByte (0xFD);
...
SD_BOOL SD_SendDataBlock (const uint8_t *buf, uint8_t tkn, uint32_t len) 
{
    ...
    /* Send Start Block Token */
       Chip_SPI_SendMidFrame(LPC_SPI0,tkn);                          //SPI_SendByte (tkn);

    /* Send data block */
    for (i = 0; i < len; i++) 
    {
      Chip_SPI_SendMidFrame(LPC_SPI0,buf[i]);                                            //SPI_SendByte (buf[i]);
    }

    /* Send 2 bytes dummy CRC */
   Chip_SPI_SendMidFrame(LPC_SPI0,0xFF); //SPI_SendByte (0xFF);
   Chip_SPI_SendMidFrame(LPC_SPI0,0xFF); //SPI_SendByte (0xFF);
